...
from transformers import AutoTokenizer, EsmConfig, TrainingArguments, Trainer, EsmForSequenceClassification, EarlyStoppingCallback
...
config: EsmConfig = EsmConfig.from_pretrained(model_name, position_embedding_type="rotary", num_labels=2)
tokenizer: AutoTokenizer = AutoTokenizer.from_pretrained(model_name)
model: EsmForSequenceClassification = EsmForSequenceClassification.from_pretrained(model_name, config=config, cache_dir="./model/")
...
def preprocess_function(examples: Dict[str, List[str]]) -> Dict[str, List[Any]]:
    return tokenizer(examples["sequence"], truncation=True, padding='max_length', max_length=20)
...
class MyTrainer(Trainer):
    def on_evaluate(self, args: TrainingArguments, state: Any, control: Any, metrics: Dict[str, float] = None, **kwargs: Any) -> None:
